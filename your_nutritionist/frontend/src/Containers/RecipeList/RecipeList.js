...
class RecipeList extends Component {
    ...
    render() {
        ...
        if (this.state.recipes.length !== 0) {
            ...
            for (
                var row_index = 0;
                row_index < Math.round(recipes.length);
                row_index++
            ) {
                let recipes_row = [];
                ...
                recipe_cards.push(
                    <Row className="recipe-row">{recipes_row}</Row>
                );
            }
            if (this.props.page) {
                console.log(this.props.page);
                let page_min = Math.max(1, this.props.page - 2);
                let page_block_start =
                    Math.floor((this.props.page - 1) / 5) * 5;
                ...
                if (this.state.recipes.length < 50) {
                    page_max = Math.max(
                        Math.min(
                            this.props.page + 2,
                            page_block_start +
                                Math.ceil(this.state.recipes.length / 10)
                        ),
                        this.props.page
                    );
                }
                for (let i = page_min; i <= page_max; i++) {
                    page_navigators.push(
                        <Button
                            ...
                            variant={
                                this.props.page === i ? "secondary" : "light"
                            }
                        >
                            {i}
                        </Button>
